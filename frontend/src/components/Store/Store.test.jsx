...
describe('Store Component', () => {
    test('renders store details correctly', async () => {
        const { findByText, getByAltText, getByText } = render(<Store store={mockStore} included={mockIncluded} />);
        ...
        const storeFooter = screen.getByText(/01\/01\/2022/).closest('.store-footer');
        expect(storeFooter).toBeInTheDocument();

        const establishmentDateElement = within(storeFooter).getByText('01/01/2022 -');
        expect(establishmentDateElement).toBeInTheDocument();

        const websiteLink = within(storeFooter).getByRole('link', { href: 'http://www.mockstore.com' });
    ...
    test('displays "No data available" message when there are no books', async () => {
        const mockStoreNoBooks = { ...mockStore, relationships: { ...mockStore.relationships, books: { data: [] } } };
        const { findByText } = render(<Store store={mockStoreNoBooks} included={mockIncluded} />);
