...
describe('StoreFooter Component', () => {
    ...
    test('renders establishment date and website correctly', () => {
        ...
        const storeFooter = screen.getByText(/15\/01\/2020/).closest('.store-footer');
        expect(storeFooter).toBeInTheDocument();

        const establishmentDateElement = within(storeFooter).getByText('15/01/2020 -');
        expect(establishmentDateElement).toBeInTheDocument();

        const websiteLink = within(storeFooter).getByRole('link', { href: 'http://www.example.com' });
